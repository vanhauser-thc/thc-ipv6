...
void help(char *prg) {
  ...
  printf("  %s [-l|-f] [-t ttl] [-s src6] [-d dst6] interface assert multicast6 [metric [sender6]]\n", prg);
  printf("  %s [-l|-f] [-t ttl] [-s src6] [-d dst6] interface join|prune neighbor6 multicast6 target6\n", prg);
  printf("\nOptions:\n");
...
int main(int argc, char *argv[]) {
  ...
  unsigned char *dst6 = NULL, *tmp6 = NULL, *src6 = NULL, *gsrc6 = NULL, *multicast6, *target6, *neighbor6;
  ...
  if (strncasecmp(argv[optind + 1], "prune", 3) == 0)
    mode = 256 + 3;
  if (strncasecmp(argv[optind + 1], "bootstrap", 4) == 0)
    mode = 4;
  if (strncasecmp(argv[optind + 1], "assert", 3) == 0)
    mode = 5;
  if (strcasecmp(argv[optind + 1], "register") == 0)
    mode = 1;
  if (strncasecmp(argv[optind + 1], "register", 9) == 0 && strlen(argv[optind + 1]) > 9)
    mode = 2; // register-stop
  if (mode == -1) {
    fprintf(stderr, "Error: no mode defined, specify hello, bootstrap, assert, join or prune\n");
    exit(-1);
  }
  if (flood && loop)
    loop = 0;
  ...
  if ((mode == 4) && argc - optind < 3) {
    fprintf(stderr, "Error: bootstrap mode needs to specify at least a BSR address\n");
    exit(-1);
  }
  if ((mode == 5) && argc - optind < 3) {
    fprintf(stderr, "Error: assert mode needs to specify at least a multicast6 address\n");
    exit(-1);
  }
  if (mode % 16 == 3) {
    if (argc - optind != 5) {
  ...
  if (mode != 1 && mode != 2 && mode < 6) {
    ...
    if (tmp_ttl == -1)
      ...
      ttl = tmp_ttl;
    if ((pkt1 = thc_create_ipv6_extended(interface, PREFER_GLOBAL, &pkt1_len, src6, dst6, ttl, 0, 0, 0, 0)) == NULL)
      return -1;
  }
  ...
  do {
    if (flood) {
      ...
      j = checksum_pseudo_header(hdr->pkt + offset + 8, hdr->pkt + offset + 24, NXT_PIM, &hdr->pkt[offset + 40], hdr->pkt_len - offset - 40);
