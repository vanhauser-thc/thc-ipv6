...
void check_packet(u_char *foo, const struct pcap_pkthdr *header, const unsigned char *data) {
  ...
  if (udp == 0 && ptr[nxt] == NXT_TCP && memcmp(dst, ptr + 8, 16) == 0 && len >= 60 + offset) {
...
//printf("rsport: %d, rdport: %d, sport %d, port %d, count %d\n", rsport, rdport, sport, port, count);
    if ((is_srcport == 1 || rdport == sport + count) && rsport == port) {
      ...
      if ((ptr[53 + offset] & 1) > 0)
        printf("-FIN");
      if ((ptr[53 + offset] & 2) > 0)
        printf("-SYN");
      if ((ptr[53 + offset] & 4) > 0)
        printf("-RST");
      if ((ptr[53 + offset] & 16) > 0)
        printf("-ACK");
      printf(" received\n");
      matched = 1;
    } // else printf("DEBUG: different tcp pkt seen from target (is_srcport %d, rdport %d == sport %d + count %d, rsport %d == port %d)\n", is_srcport, rdport, sport, count, rsport, port);
  }
  ...
  if (ptr[nxt] == NXT_ICMP6 && (ptr[40 + offset] == ICMP6_UNREACH || ptr[40 + offset] == ICMP6_PARAMPROB) && len >= 96 + poffset + offset) {
    ...
    if (ptype >= 0) {
      if (ptype == NXT_FRAG && poffset == 0) {
        if (ptr[54 + offset] != NXT_FRAG && ptr[54 + offset] != NXT_DST)
          return;
      } else {
        if (ptr[54 + offset] != ptype)
          return;
        if (udp == 0 && ptr[54 + poffset2 + offset] != NXT_TCP)
          return;
        if (udp == 1 && ptr[54 + poffset2 + offset] != NXT_UDP)
          return;
      }
    } else {
      if (udp == 0 && ptr[54 + offset] != NXT_TCP)
        return;
      if (udp == 1 && ptr[54 + offset] != NXT_UDP)
        return;
    }
...
int main(int argc, char *argv[]) {
  ...
  if (only == ++count || only == 0) {
    ...
    while (thc_pcap_check(p, (char *) ignoreit, NULL) > 0);
    hdr = (thc_ipv6_hdr *) pkt;
    thc_send_as_fragment6(interface, src, dst, NXT_FRAG, hdr->pkt + 40 + offset, hdr->pkt_len - 40 - offset, 1240);
    thc_send_as_fragment6(interface, src, dst, NXT_FRAG, hdr->pkt + 40 + offset, hdr->pkt_len - 40 - offset, 1240);
    check_for_reply();
    pkt = thc_destroy_packet(pkt);
    curr++;
  ...
  if (only == ++count || only == 0) {
    ...
    while (thc_pcap_check(p, (char *) ignoreit, NULL) > 0);
    hdr = (thc_ipv6_hdr *) pkt;
    thc_send_as_fragment6(interface, src, dst, NXT_DST, hdr->pkt + 40 + offset, hdr->pkt_len - 40 - offset, 1240);
    thc_send_as_fragment6(interface, src, dst, NXT_DST, hdr->pkt + 40 + offset, hdr->pkt_len - 40 - offset, 1240);
    check_for_reply();
    pkt = thc_destroy_packet(pkt);
    curr++;
  ...
  if (only == ++count || only == 0) {
    ...
    if (thc_add_hdr_fragment(pkt2, &pkt_len2, 2464 / 8, 0, sport + count) == -1)
      return -1;
    if (thc_add_data6(pkt2, &pkt_len2, NXT_ICMP6, hdr->pkt + 2464 + 40 + offset, hdr->pkt_len - 2464 - 40 - do_hdr_size) == -1)
      return -1;
    if (thc_generate_and_send_pkt(interface, srcmac, dstmac, pkt2, &pkt_len2) == -1)
      return -1;
    pkt2 = thc_destroy_packet(pkt2);
  ...
  if (only == ++count || only == 0) {
    ...
    // interface, ip6, dst, type, frhdr->pkt + 40 + myoff, frhdr->pkt_len - 40 - myoff, 1232, 0
    thc_send_as_overlapping_first_fragment6(interface, src, dst, NXT_ICMP6, hdr->pkt + 40, hdr->pkt_len - 40, 1232, 0);
  ...
  if (only == ++count || only == 0) {
    ...
    // interface, ip6, dst, type, frhdr->pkt + 40 + myoff, frhdr->pkt_len - 40 - myoff, 1232, 0
    thc_send_as_overlapping_last_fragment6(interface, src, dst, NXT_ICMP6, hdr->pkt + 40, hdr->pkt_len - 40, 1232, 0);
  ...
  if (only == ++count || only == 0) {
    ...
    if (thc_add_hdr_dst(pkt, &pkt_len, buf, 6) == -1)
      return -1;
    if (udp == 0) {
      if (thc_add_tcp(pkt, &pkt_len, sport + count, port, sport + count, 0, TCP_SYN, 0x3840, 0, NULL, 0, buf, 0) == -1)
        return -1;
    } else {
    ...
    if (thc_add_hdr_dst(pkt3, &pkt_len3, buf, 6) == -1)
      return -1;
    if (thc_add_icmp6(pkt3, &pkt_len3, ICMP6_ECHOREQUEST, 0, sport + count, buf, 8, 0) == -1)
        return -1;
  ...
  if (only == ++count || only == 0) {
    ...
    if (thc_add_hdr_dst(pkt, &pkt_len, buf, 6) == -1)
      return -1;
    if (udp == 0) {
      if (thc_add_tcp(pkt, &pkt_len, sport + count, port, sport + count, 0, TCP_SYN, 0x3840, 0, NULL, 0, buf, 40) == -1)
        return -1;
    } else {
    ...
    if (thc_add_hdr_dst(pkt3, &pkt_len3, buf, 6) == -1)
      return -1;
    if (thc_add_icmp6(pkt3, &pkt_len3, ICMP6_ECHOREQUEST, 0, sport + count, buf, 32, 0) == -1)
        return -1;
    if (thc_generate_pkt(interface, srcmac, dstmac, pkt3, &pkt_len3) == -1)
      return -1;
    hdr3 = (thc_ipv6_hdr *) pkt3;
    ...
    if (thc_add_hdr_fragment(pkt2, &pkt_len2, 2, 1, sport + count) == -1)
      return -1;
    if (thc_add_data6(pkt2, &pkt_len2, NXT_DST, hdr3->pkt + 40 + offset + 16, 32) == -1)
      return -1;
    if (thc_generate_and_send_pkt(interface, srcmac, dstmac, pkt2, &pkt_len2) == -1)
      return -1;
    pkt2 = thc_destroy_packet(pkt2);
    ...
    if (thc_add_hdr_fragment(pkt2, &pkt_len2, 2, 1, sport + count) == -1)
      return -1;
    if (thc_add_data6(pkt2, &pkt_len2, NXT_DST, hdr->pkt + 40 + offset + 16, 32) == -1)
      return -1;
    if (thc_generate_and_send_pkt(interface, srcmac, dstmac, pkt2, &pkt_len2) == -1)
      return -1;
    pkt2 = thc_destroy_packet(pkt2);
    ...
    if (thc_add_hdr_fragment(pkt2, &pkt_len2, 6, 0, sport + count) == -1)
      return -1;
    if (thc_add_data6(pkt2, &pkt_len2, NXT_DST, hdr->pkt + 40 + offset + 48, hdr->pkt_len - 40 - offset - 48) == -1)
      return -1;
    if (thc_generate_and_send_pkt(interface, srcmac, dstmac, pkt2, &pkt_len2) == -1)
      return -1;
    pkt2 = thc_destroy_packet(pkt2);
