...
//                    0x68, 0x6f, 0x73, 0x74, 0x00, 0x00, 0x01, 0x00, 0x01 };
  ...
  if (verbose) {
    timeval = time(NULL);
    printf("Starting alive6 %s (c) 2017 by van Hauser / THC at %s\n", VERSION, ctime(&timeval));
  }
...
//printf("ptr: %s, line %s, cur_dst %s, multicast6 %s\n", ptr, line, cur_dst, multicast6);
        ...
        inc_step = 1;
        inc_next = 0;
//printf("%d S: %04x-%04x=%04x %04x-%04x=%04x %04x-%04x=%04x %04x-%04x=%04x %04x-%04x=%04x %04x-%04x=%04x %04x-%04x=%04x %04x-%04x=%04x\n", ok, fip1, tip1, cip1, fip2, tip2, cip2, fip3, tip3, cip3, fip4, tip4, cip4, fip5, tip5, cip5, fip6, tip6, cip6, fip7, tip7, cip7, fip8, tip8, cip8);
        ...
        if (inc_step || inc_next) // we are done
          ok = 0;
//printf("%d E: %04x-%04x=%04x %04x-%04x=%04x %04x-%04x=%04x %04x-%04x=%04x %04x-%04x=%04x %04x-%04x=%04x %04x-%04x=%04x %04x-%04x=%04x\n", ok, fip1, tip1, cip1, fip2, tip2, cip2, fip3, tip3, cip3, fip4, tip4, cip4, fip5, tip5, cip5, fip6, tip6, cip6, fip7, tip7, cip7, fip8, tip8, cip8);
    ...
    if (ok != 0 && cur_dst != NULL) {
      if (renew) {
        while ((src6 = thc_get_own_ipv6(interface, cur_dst, prefer)) == NULL || (prefer == PREFER_GLOBAL && src6[0] > 0xfd)) {
          fprintf(stderr, "Error: no global IPv6 address found for interface %s, sleeping for 60 seconds and waiting for one! Next check in 60 seconds ...\n", interface);
          sleep(60);
        }
      }
      if (debug)
        ...
        if (synports[0] != -1 || portscan) {
          ...
          while ((portscan > 0 && portscan < 65536) || (synports[i] != -1 && i < MAX_PORTS)) {
            ...
            if (portscan) {
              ...
              if (thc_add_tcp(pkt, &pkt_len, sp, synports[i] % 65536, (sp << 16) + sp, 0, TCP_SYN, 5760, 0, tcp_opt, TCP_OPT_LEN, NULL, 0) < 0)
                return -1;
            }
            ...
            if (router6 != NULL) {
              ...
              while(thc_send_pkt(interface, pkt, &pkt_len) < 0) {
                ...
                if (errcnt > 0)
                  ...
                  printf("Error: unable to send packet to network, waiting for 60 seconds to recover ...\n");
                  sleep(60);
                  errcnt = 0;
                }
                errcnt++;
